...
void do_mstat( CHAR_DATA * ch, char *argument )
{
   ...
   ch_printf( ch,
              ...
              victim->top_level, victim->race, victim->alignment, get_character_defence( victim ), victim->gold );
...
void do_advance( CHAR_DATA * ch, char *argument )
{
   ...
   if( (get_trust( ch ) <= get_trust( victim ) && ch->top_level< LEVEL_DEMI) || ((ch == victim) && (ch->top_level < LEVEL_DEMI)))
   {
...
void do_advancelevel( CHAR_DATA * ch, char *argument )
{
   char arg1[MAX_INPUT_LENGTH];
   char arg2[MAX_INPUT_LENGTH];
   CHAR_DATA *victim;
   ...
   {
      send_to_char( "Syntax: advancelevel <char> <level>.\n\r", ch );
      return;
   }
   ...
   if( (get_trust( ch ) <= get_trust( victim ) && ch->top_level< LEVEL_DEMI) || ((ch == victim) && (ch->top_level < LEVEL_DEMI)) || victim->top_level == LEVEL_SUPREME)
   {
      send_to_char( "You can't do that.\n\r", ch );
      return;
   }

   if( ( level = atoi( arg2 ) ) < 1 || level > LEVEL_SUPREME )
   {
	   char buf[50];
	   sprintf(buf, "Level must be 1 to %d.\r\r", LEVEL_SUPREME);
      send_to_char(buf,ch);
      return;
   }
   ...
   for( iLevel = victim->top_level; iLevel < level; iLevel++ )
   {
	   victim->top_level++;
	   send_to_char("You have raised a level. You feel more powerful!\n\r", victim);
   }
   return;
}
...
void do_trust( CHAR_DATA * ch, char *argument )
{
   ...
   if( (get_trust( victim ) >= get_trust( ch )) && (ch->top_level < LEVEL_IMMORTAL ) )
   {
      send_to_char( "You can't do that.\n\r", ch );
      return;
   }
...
void do_roomflags(CHAR_DATA *ch, char *argument) {
	...
	if (IS_SET(ch->act, PLR_ROOMFLAGS)) {
		...
		send_to_char("Roomflags off.\r\r", ch);
	} else {
		SET_BIT(ch->act, PLR_ROOMFLAGS);
		send_to_char("Roomflags on.\r\r", ch);
	}
	return;
}
*/
