                        ...
                        ( ship->class == LAND_SPEEDER ? "Speeder" :
                          ( ship->class == WHEELED ? "Wheeled Transport" :
                            ( ship->class == LAND_CRAWLER ? "Crawler" :
                              ( ship->class == WALKER ? "Walker" : "Unknown" ) ) ) ) ) ) ) ) ), ship->name, ship->filename );
...
void do_openhatch( CHAR_DATA * ch, char *argument )
{
   ...
   if( !check_pilot( ch, ship ) && !IS_IMMORTAL(ch) )
   {
      send_to_char( "&RHey! Thats not your ship!\n\r", ch );
      return;
   }
...
void do_hyperspace( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( ship = ship_from_pilotseat( ch->in_room->vnum ) ) == NULL )
   {
      send_to_char( "&RYou aren't in the pilot's seat.\n\r", ch );
      return;
   }
   ...
   if( ship->shipstate == SHIP_HYPERSPACE )
   {
      send_to_char( "&RYou are already travelling at lightspeed!\n\r", ch );
      return;
   }
   if( ship->shipstate == SHIP_DISABLED )
   {
      send_to_char( "&RThe ship's drive is disabled. Unable to manuever.\n\r", ch );
      return;
   }
   if( ship->shipstate == SHIP_DOCKED )
   {
...
void do_target( CHAR_DATA * ch, char *argument )
{
   ...
   switch ( ch->substate )
   {
      default:
         ...
         {
            send_to_char( "&RYou must be in the gunner's seat or turret of a ship to do that!\n\r", ch );
            return;
         }
         ...
         /* let them target their own ships if they really want to
   ...
   {
      send_to_char( "&RThe ship has left the starsytem. Targetting aborted.\n\r", ch );
      return;
   }
...
void do_fire( CHAR_DATA * ch, char *argument )
{
   ...
   if( ( ship = ship_from_turret( ch->in_room->vnum ) ) == NULL )
   {
      send_to_char( "&RYou must be in the gunner's chair or turret of a ship to do that!\n\r", ch );
      return;
   }
   ...
   if( ship->energy < 5 )
   {
      send_to_char( "&RThere's not enough energy left to fire!\n\r", ch );
      return;
   }
   ...
   if( ch->in_room->vnum == ship->gunseat && !str_prefix( argument, "lasers" ) )
   {
      ...
      if( ship->statet0 == LASER_DAMAGED )
      {
         send_to_char( "&RThe ship's main laser is damaged.\n\r", ch );
         return;
      }
   ...
   if( ch->in_room->vnum == ship->gunseat && !str_prefix( argument, "missile" ) )
   {
      if( ship->missilestate == MISSILE_DAMAGED )
      {
         send_to_char( "&RThe ship's missile launchers are damaged.\n\r", ch );
         return;
      }
      if( ship->missiles <= 0 )
      {
   ...
   if( ch->in_room->vnum == ship->gunseat && !str_prefix( argument, "torpedo" ) )
   {
      if( ship->missilestate == MISSILE_DAMAGED )
      {
         send_to_char( "&RThe ship's missile launchers are dammaged.\n\r", ch );
         return;
      }
      if( ship->torpedos <= 0 )
      {
   ...
   if( ch->in_room->vnum == ship->turret2 && !str_prefix( argument, "lasers" ) )
   {
      if( ship->statet2 == LASER_DAMAGED )
      {
         send_to_char( "&RThe ship's turret is damaged.\n\r", ch );
         return;
      }
